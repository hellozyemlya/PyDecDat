...
class Dat:
    def __init__(self, dat_content, encoding):
        DatSymbol.next_id = 0
        self.encoding = encoding

        if isinstance(dat_content, str):
            with open(dat_content, 'rb') as dat_file:
                file = FileWrapper(dat_file.read(), encoding=encoding)
        elif isinstance(dat_content, bytes):
            file = FileWrapper(dat_content, encoding=encoding)
        else:
            raise ValueError(f"dat_content must be dat file 'bytes' or dat file path 'str'")
